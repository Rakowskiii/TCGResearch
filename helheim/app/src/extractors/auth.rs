        ...
        S: Sized + Send + Sync + Debug,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        let app_state: crate::appstate::AppState = parts.extract_with_state::<crate::appstate::AppState, _>(state)
            .await?;

        match parts.headers.get(axum::http::header::AUTHORIZATION) {
            Some(bearer) => {
                let auth = bearer.to_str().unwrap_or("").to_string().strip_prefix("Bearer ").unwrap_or("").to_string();
